...
--
-- One row per service request. `content` is the retrieval unit (a concatenation
-- of complaint_type, descriptor, address, and resolution_description, see the
-- ingestion script). `meta` carries the structured attributes the LLM filters
-- on. `tree` is nyc.<borough>.<agency>.<complaint_type>. `temporal` is the
-- request's [created_date, closed_date) lifetime (point-in-time when the
-- request is still open). `geom` is the incident location in WGS84.
