// search.ts — direct transcription of the tutorial's searchDocuments, plus
// a few notes about the parts that don't compile as written.
...
// `metersProj` returns a sql fragment that may be empty. The tutorial uses
// `sql``` for the empty case, which works.
...
async function fetchByIds(ids: string[], p: SearchParams) {
  if (ids.length === 0) return [] as SearchResult[];
  return sql<SearchResult[]>`
