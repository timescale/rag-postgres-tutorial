// searchDocuments: parameterized retrieval that fuses BM25 + vector with RRF
// and accepts ltree / JSONB / temporal / geo filters in a single SQL pass.
//
// Code transplanted from blog_rag_tutorial.md Step 6h.
...
export interface SearchParams {
  ...
  fulltext?: string;
  tree?: string;
  meta?: Record<string, any>;          // JSONB containment; postgres-js JSONValue-compatible
...
function buildFilters(p: SearchParams) {
  ...
  // postgres-js requires `sql.json(...)` to send a plain object as a JSONB
  // parameter. Don't `JSON.stringify` yourself — postgres-js will then
  // re-encode the string, yielding a doubly-quoted JSON scalar that
  // matches nothing.
